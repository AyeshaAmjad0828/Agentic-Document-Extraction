...
from src.utils.parallel_processing import process_single_page  # New import
...
def process_document(file_path: str, extraction_groundtruth: dict, output_dir: str = None, 
                    schema_groundtruth: dict = None, max_workers: int = None) -> dict:
    """
        ...
        for result in sorted(page_results, key=lambda x: x['page_num']):
            ...
            if isinstance(page_data['best_output'], str):
                try:
                    ...
                    print(f"Warning: Could not parse JSON from page {result['page_num'] + 1}")
                    continue
            else:
